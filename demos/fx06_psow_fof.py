...
def main():
    ...
    psow_dupl_2 = cycdp.psow_dupl(buf, repeat_count=2)
    print(f"PSOW duplicate 2x (stutter stretch)")
    outputs["fx06_psow_dupl_2x.wav"] = psow_dupl_2

    psow_dupl_4 = cycdp.psow_dupl(buf, repeat_count=4)
    ...
    grab_early = cycdp.psow_grab(buf, time=min(1.0, duration * 0.1), duration=4.0, density=1.0)
    print(f"Grab early, sustain 4s")
    outputs["fx06_psow_grab_early.wav"] = grab_early

    grab_mid = cycdp.psow_grab(buf, time=duration / 2, duration=4.0, density=1.0)
    print(f"Grab middle, sustain 4s")
    outputs["fx06_psow_grab_mid.wav"] = grab_mid

    # Grab with pitch change via density
    grab_high = cycdp.psow_grab(buf, time=duration * 0.25, duration=4.0, density=2.0)
    print(f"Grab with density 2.0 (octave up)")
    outputs["fx06_psow_grab_octave_up.wav"] = grab_high

    grab_low = cycdp.psow_grab(buf, time=duration * 0.25, duration=4.0, density=0.5)
    ...
    fof_up_preserve = cycdp.fofex_repitch(buf, pitch_shift=7.0, preserve_formants=True)
    print(f"FOF repitch +7 (formants preserved)")
    outputs["fx06_fof_up_preserve.wav"] = fof_up_preserve

    fof_up_shift = cycdp.fofex_repitch(buf, pitch_shift=7.0, preserve_formants=False)
    print(f"FOF repitch +7 (formants shifted)")
    outputs["fx06_fof_up_shift.wav"] = fof_up_shift

    fof_down_preserve = cycdp.fofex_repitch(buf, pitch_shift=-7.0, preserve_formants=True)
    print(f"FOF repitch -7 (formants preserved)")
    outputs["fx06_fof_down_preserve.wav"] = fof_down_preserve

    fof_down_shift = cycdp.fofex_repitch(buf, pitch_shift=-7.0, preserve_formants=False)
    print(f"FOF repitch -7 (formants shifted)")
    outputs["fx06_fof_down_shift.wav"] = fof_down_shift

    # Extreme pitch shifts
    fof_extreme_up = cycdp.fofex_repitch(buf, pitch_shift=12.0, preserve_formants=True)
    print(f"FOF repitch +12 (octave up, formants preserved)")
    outputs["fx06_fof_octave_up.wav"] = fof_extreme_up

    fof_extreme_down = cycdp.fofex_repitch(buf, pitch_shift=-12.0, preserve_formants=True)
    ...
    hover_slow = cycdp.hover(buf, frequency=3.0, location=0.5, frq_rand=0.2, loc_rand=0.1)
    print(f"Slow hover (3 Hz)")
    outputs["fx06_hover_slow.wav"] = hover_slow

    hover_fast = cycdp.hover(buf, frequency=20.0, location=0.5, frq_rand=0.3)
    print(f"Fast hover (20 Hz, tremolo-like)")
    outputs["fx06_hover_fast.wav"] = hover_fast

    hover_explore = cycdp.hover(buf, frequency=1.0, location=0.3, loc_rand=0.5,
    ...
    # Phase vocoder pitch
    pv_pitch = cycdp.pitch_shift(buf, semitones=5.0)
    print(f"Phase vocoder +5")
    outputs["fx06_compare_pv.wav"] = pv_pitch

    # FOF pitch (preserve formants)
    fof_pitch = cycdp.fofex_repitch(buf, pitch_shift=5.0, preserve_formants=True)
    print(f"FOF +5 (preserve formants)")
    outputs["fx06_compare_fof_preserve.wav"] = fof_pitch

    # Granular pitch
    gran_pitch = cycdp.brassage(buf, velocity=1.0, pitch_shift=5.0, grainsize_ms=40.0)
    print(f"Granular +5")
