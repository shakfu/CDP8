...
morphed = cycdp.morph(brass, flute, morph_start=0.0, morph_end=1.0)
...
partial = cycdp.morph(brass, flute, morph_start=0.3, morph_end=0.7)
print(f"Partial morph (30%-70%): stays brass, morphs, stays flute")

# Fast morph with exponent
fast_morph = cycdp.morph(brass, flute, morph_start=0.0, morph_end=1.0, exponent=0.5)
print(f"Fast-start morph (exponent=0.5): quick transition then slow")

# Slow morph with exponent
slow_morph = cycdp.morph(brass, flute, morph_start=0.0, morph_end=1.0, exponent=2.0)
...
# Short glide
short_glide = cycdp.morph_glide(brass, flute, duration=0.5)
print(f"Short glide (0.5s): {short_glide.frame_count} frames")

# Long glide
long_glide = cycdp.morph_glide(brass, flute, duration=2.0)
print(f"Long glide (2.0s): {long_glide.frame_count} frames")

# Vowel glide: "ah" to "ee"
vowel_glide = cycdp.morph_glide(vowel_a, vowel_i, duration=1.0)
...
cross1 = cycdp.cross_synth(brass, flute, mode=0, mix=1.0)
...
cross2 = cycdp.cross_synth(brass, flute, mode=1, mix=1.0)
...
partial_cross = cycdp.cross_synth(brass, flute, mode=0, mix=0.5)
...
native_glide = cycdp.morph_glide_native(brass, flute, duration=1.0, fft_size=1024)
...
bridge_no_norm = cycdp.morph_bridge_native(brass, flute, mode=0)
print(f"Bridge mode 0 (no normalization): {bridge_no_norm.frame_count} frames")

bridge_norm_min = cycdp.morph_bridge_native(brass, flute, mode=1)
print(f"Bridge mode 1 (normalize to minimum)")

bridge_norm_file1 = cycdp.morph_bridge_native(brass, flute, mode=2)
print(f"Bridge mode 2 (normalize to file 1 level)")

# Bridge with offset (start file2 later)
bridge_offset = cycdp.morph_bridge_native(brass, flute, mode=0, offset=0.1)
print(f"Bridge with 0.1s offset: file2 starts later")

# Bridge with custom interpolation range
bridge_custom = cycdp.morph_bridge_native(brass, flute, mode=0,
...
# Linear morph
linear_morph = cycdp.morph_native(brass, flute, mode=0)
print(f"Native morph linear (mode=0): {linear_morph.frame_count} frames")

# Cosine morph (smoother)
cosine_morph = cycdp.morph_native(brass, flute, mode=1)
...
amp_fast = cycdp.morph_native(brass, flute, mode=0,
...
freq_first = cycdp.morph_native(brass, flute, mode=0,
...
curved_morph = cycdp.morph_native(brass, flute, mode=0,
...
morph_a_i = cycdp.morph(vowel_a, vowel_i)
morph_i_o = cycdp.morph(vowel_i, vowel_o)
vowel_chain = cycdp.concat([morph_a_i, morph_i_o])
print(f"Vowel chain 'a'->'i'->'o': {vowel_chain.frame_count} frames")

# Hybrid sound: cross-synth then morph
print("\n--- Hybrid Sound Creation ---")
hybrid = cycdp.cross_synth(brass, flute, mode=0, mix=0.7)
hybrid_morphed = cycdp.morph(hybrid, vowel_a, morph_start=0.2, morph_end=0.8)
print(f"Hybrid (brass+flute) morphed to vowel: {hybrid_morphed.frame_count} frames")

# Self-morphing with pitch shift
print("\n--- Self-Morphing ---")
brass_shifted = cycdp.pitch_shift(brass, semitones=7.0)  # Up a fifth
self_morph = cycdp.morph(brass, brass_shifted)
...
1. MATCHING DURATIONS:
   ...
   - Use time_stretch to match lengths if needed:
     buf2_stretched = cycdp.time_stretch(buf2, factor=len(buf1)/len(buf2))
...
outputs = {
    "07_brass.wav": cycdp.normalize(brass),
    "07_flute.wav": cycdp.normalize(flute),
    "07_vowel_a.wav": cycdp.normalize(vowel_a),
    "07_vowel_i.wav": cycdp.normalize(vowel_i),
    "07_morph_brass_flute.wav": cycdp.normalize(morphed),
    "07_morph_glide_long.wav": cycdp.normalize(long_glide),
    "07_vowel_glide.wav": cycdp.normalize(vowel_glide),
    "07_cross_synth.wav": cycdp.normalize(cross1),
    "07_native_glide.wav": cycdp.normalize(native_glide),
    "07_native_bridge.wav": cycdp.normalize(bridge_no_norm),
    "07_native_morph_linear.wav": cycdp.normalize(linear_morph),
    "07_native_morph_cosine.wav": cycdp.normalize(cosine_morph),
    "07_vowel_chain.wav": cycdp.normalize(vowel_chain),
    "07_hybrid_morphed.wav": cycdp.normalize(hybrid_morphed),
}
