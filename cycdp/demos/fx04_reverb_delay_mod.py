...
def main():
    ...
    reverb_small = cycdp.reverb(buf, decay_time=0.8, damping=0.6, mix=0.3)
    print(f"Small room (0.8s decay)")
    outputs["fx04_reverb_small.wav"] = reverb_small

    reverb_medium = cycdp.reverb(buf, decay_time=2.0, damping=0.4, mix=0.4)
    print(f"Medium hall (2s decay)")
    outputs["fx04_reverb_medium.wav"] = reverb_medium

    reverb_large = cycdp.reverb(buf, decay_time=4.0, damping=0.3, mix=0.5)
    print(f"Large hall (4s decay)")
    outputs["fx04_reverb_large.wav"] = reverb_large

    reverb_huge = cycdp.reverb(buf, decay_time=8.0, damping=0.2, mix=0.6)
    print(f"Cathedral (8s decay)")
    outputs["fx04_reverb_cathedral.wav"] = reverb_huge

    reverb_dark = cycdp.reverb(buf, decay_time=3.0, damping=0.8, mix=0.5)
    print(f"Dark reverb (high damping)")
    outputs["fx04_reverb_dark.wav"] = reverb_dark

    reverb_bright = cycdp.reverb(buf, decay_time=3.0, damping=0.1, mix=0.5)
    ...
    delay_short = cycdp.delay(buf, delay_ms=125.0, feedback=0.3, mix=0.4)
    print(f"Short delay (125ms, 1/8 note at 120bpm)")
    outputs["fx04_delay_short.wav"] = delay_short

    delay_quarter = cycdp.delay(buf, delay_ms=500.0, feedback=0.4, mix=0.4)
    print(f"Quarter note delay (500ms)")
    outputs["fx04_delay_quarter.wav"] = delay_quarter

    delay_long = cycdp.delay(buf, delay_ms=750.0, feedback=0.5, mix=0.35)
    print(f"Dotted quarter delay (750ms)")
    outputs["fx04_delay_dotted.wav"] = delay_long

    delay_heavy = cycdp.delay(buf, delay_ms=400.0, feedback=0.7, mix=0.5)
    print(f"Heavy feedback delay (70%)")
    outputs["fx04_delay_heavy_fb.wav"] = delay_heavy

    delay_slapback = cycdp.delay(buf, delay_ms=80.0, feedback=0.1, mix=0.3)
    ...
    trem_slow = cycdp.tremolo(buf, freq=2.0, depth=0.6)
    print(f"Slow tremolo (2 Hz)")
    outputs["fx04_tremolo_slow.wav"] = trem_slow

    trem_medium = cycdp.tremolo(buf, freq=6.0, depth=0.7)
    print(f"Medium tremolo (6 Hz)")
    outputs["fx04_tremolo_medium.wav"] = trem_medium

    trem_fast = cycdp.tremolo(buf, freq=12.0, depth=0.5)
    ...
    chorus_light = cycdp.chorus(buf, rate=1.0, depth_ms=15.0, mix=0.4)
    print(f"Light chorus")
    outputs["fx04_chorus_light.wav"] = chorus_light

    chorus_heavy = cycdp.chorus(buf, rate=0.5, depth_ms=30.0, mix=0.5)
    ...
    flanger_slow = cycdp.flanger(buf, rate=0.2, depth_ms=5.0, feedback=0.5)
    print(f"Slow flanger")
    outputs["fx04_flanger_slow.wav"] = flanger_slow

    flanger_fast = cycdp.flanger(buf, rate=1.0, depth_ms=3.0, feedback=0.6)
    print(f"Fast flanger")
    outputs["fx04_flanger_fast.wav"] = flanger_fast

    flanger_jet = cycdp.flanger(buf, rate=0.1, depth_ms=8.0, feedback=0.8)
    ...
    # Delay into reverb
    delay_verb = cycdp.delay(buf, delay_ms=375.0, feedback=0.4, mix=0.3)
    delay_verb = cycdp.reverb(delay_verb, decay_time=2.5, damping=0.4, mix=0.4)
    print(f"Delay into reverb")
    outputs["fx04_delay_into_reverb.wav"] = delay_verb

    # Chorus + reverb (lush)
    lush = cycdp.chorus(buf, rate=0.8, depth_ms=20.0, mix=0.4)
    lush = cycdp.reverb(lush, decay_time=3.0, damping=0.3, mix=0.45)
