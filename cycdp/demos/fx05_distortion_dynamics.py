...
def main():
    ...
    fractal_light = cycdp.distort_fractal(buf, scaling=0.7)
    print(f"Fractal light (0.7 scaling)")
    outputs["fx05_fractal_light.wav"] = fractal_light

    fractal_heavy = cycdp.distort_fractal(buf, scaling=0.3)
    ...
    crush_12 = cycdp.bitcrush(buf, bit_depth=12)
    print(f"12-bit (subtle)")
    outputs["fx05_bitcrush_12.wav"] = crush_12

    crush_8 = cycdp.bitcrush(buf, bit_depth=8)
    print(f"8-bit (retro)")
    outputs["fx05_bitcrush_8.wav"] = crush_8

    crush_4 = cycdp.bitcrush(buf, bit_depth=4)
    ...
    lp_dark = cycdp.filter_lowpass(buf, cutoff_freq=500.0)
    print(f"Lowpass 500 Hz (dark)")
    outputs["fx05_lowpass_500.wav"] = lp_dark

    lp_warm = cycdp.filter_lowpass(buf, cutoff_freq=2000.0)
    print(f"Lowpass 2000 Hz (warm)")
    outputs["fx05_lowpass_2k.wav"] = lp_warm

    hp_thin = cycdp.filter_highpass(buf, cutoff_freq=1000.0)
    print(f"Highpass 1000 Hz (thin)")
    outputs["fx05_highpass_1k.wav"] = hp_thin

    hp_telephone = cycdp.filter_highpass(buf, cutoff_freq=300.0)
    hp_telephone = cycdp.filter_lowpass(hp_telephone, cutoff_freq=3000.0)
    print(f"Telephone (300-3000 Hz bandpass)")
    outputs["fx05_telephone.wav"] = hp_telephone

    bp_nasal = cycdp.filter_bandpass(buf, low_freq=800.0, high_freq=1500.0)
    print(f"Bandpass 800-1500 Hz (nasal)")
    outputs["fx05_bandpass_nasal.wav"] = bp_nasal

    notch = cycdp.filter_notch(buf, center_freq=1000.0, width_hz=200.0)
    ...
    lofi = cycdp.bitcrush(buf, bit_depth=10)
    lofi = cycdp.filter_lowpass(lofi, cutoff_freq=4000.0)
    lofi = cycdp.compressor(lofi, threshold_db=-15.0, ratio=3.0)
    print(f"Lo-fi chain")
    outputs["fx05_lofi.wav"] = lofi

    # Industrial: waveset + fractal + compression
    industrial = cycdp.distort_repeat(buf, multiplier=2)
    industrial = cycdp.distort_fractal(industrial, scaling=0.5)
    industrial = cycdp.compressor(industrial, threshold_db=-10.0, ratio=6.0)
