...
blurred = pycdp.spectral_blur(buf, blur_time=0.1)  # blur_time in seconds
...
focused = pycdp.spectral_focus(buf, center_freq=800.0, bandwidth=200.0, gain_db=6.0)
...
compressed = pycdp.time_stretch(buf, factor=0.5)
...
# Shift up one octave
shifted_up = pycdp.pitch_shift(buf, semitones=12.0)
print(f"Applied pitch shift: +12 semitones (octave up)")

# Shift down a fifth
shifted_down = pycdp.pitch_shift(buf, semitones=-7.0)
print(f"Applied pitch shift: -7 semitones (fifth down)")

# Micro-tuning
detuned = pycdp.pitch_shift(buf, semitones=0.5)
...
spec_stretched = pycdp.spectral_stretch(buf, max_stretch=2.0, freq_divide=500.0)
...
cleaned = pycdp.spectral_clean(buf, threshold_db=-40.0)
...
hilited = pycdp.spectral_hilite(buf, threshold_db=-20.0, boost_db=6.0)
print(f"Applied spectral hilite: boost peaks by 6dB")

# Spectral fold - mirror frequencies above fold point
folded = pycdp.spectral_fold(buf, fold_freq=2000.0)
...
frozen = pycdp.freeze(buf, start_time=0.1, end_time=0.2, duration=2.0)
...
frozen_scatter = pycdp.freeze(buf, start_time=0.1, end_time=0.2,
...
result = buf
result = pycdp.time_stretch(result, factor=1.5)   # Slightly longer
result = pycdp.spectral_blur(result, blur_time=0.05)  # Soften
result = pycdp.pitch_shift(result, semitones=-5.0)     # Down a fourth
...
blurred_hires = pycdp.spectral_blur(buf, blur_time=0.1, fft_size=4096)
print(f"\nHigh-resolution blur (FFT=4096): cleaner frequencies")

blurred_lores = pycdp.spectral_blur(buf, blur_time=0.1, fft_size=512)
...
outputs = {
    "03_source.wav": pycdp.normalize(buf),
    "03_blur.wav": pycdp.normalize(blurred),
    "03_focus.wav": pycdp.normalize(focused),
    "03_time_stretch_2x.wav": pycdp.normalize(stretched),
    "03_time_compress_half.wav": pycdp.normalize(compressed),
    "03_pitch_up_octave.wav": pycdp.normalize(shifted_up),
    "03_pitch_down_fifth.wav": pycdp.normalize(shifted_down),
    "03_spectral_shift_100hz.wav": pycdp.normalize(shifted_hz),
    "03_spectral_stretch.wav": pycdp.normalize(spec_stretched),
    "03_spectral_fold.wav": pycdp.normalize(folded),
    "03_freeze.wav": pycdp.normalize(frozen),
    "03_effect_chain.wav": pycdp.normalize(result),
}
