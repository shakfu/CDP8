#!/usr/bin/env python3
"""
Demo 1: Basic Operations with pycdp
...
print(f"pycdp version: {pycdp.version()}")
print()
...
louder = pycdp.gain(buf, 2.0)
print(f"Applied 2x gain: peak = {pycdp.peak(louder)[0]:.3f}")

# Apply gain in decibels
boosted = pycdp.gain_db(buf, 6.0)  # +6 dB (approximately 2x)
print(f"Applied +6 dB: peak = {pycdp.peak(boosted)[0]:.3f}")

# Normalize to unity (peak = 1.0)
normalized = pycdp.normalize(buf)
print(f"Normalized: peak = {pycdp.peak(normalized)[0]:.3f}")

# Normalize to specific level in dB
normalized_db = pycdp.normalize_db(buf, -3.0)  # -3 dB below unity
print(f"Normalized to -3 dB: peak = {pycdp.peak(normalized_db)[0]:.3f}")
...
# Reverse
reversed_buf = pycdp.reverse(buf)
...
buf_copy = pycdp.Buffer.from_memoryview(
    array.array('f', [buf[i] for i in range(buf.frame_count)]),
    channels=1, sample_rate=sample_rate
)
pycdp.fade_in(buf_copy, duration=0.1)  # 100ms = 0.1s
print(f"Applied 100ms fade in")

# Fade out
buf_copy2 = pycdp.Buffer.from_memoryview(
    array.array('f', [buf[i] for i in range(buf.frame_count)]),
    channels=1, sample_rate=sample_rate
)
pycdp.fade_out(buf_copy2, duration=0.1)
print(f"Applied 100ms fade out")

# Concatenate buffers
concatenated = pycdp.concat([buf, reversed_buf])
...
# Pan mono to stereo
panned_center = pycdp.pan(buf, 0.0)   # Center
panned_left = pycdp.pan(buf, -1.0)    # Hard left
panned_right = pycdp.pan(buf, 1.0)    # Hard right
...
auto_panned = pycdp.pan_envelope(buf, pan_envelope)
print(f"Applied pan envelope: L -> R -> L")

# Mirror stereo (swap L/R)
mirrored = pycdp.mirror(panned_center)
print(f"Mirrored stereo channels")

# Narrow stereo width
narrowed = pycdp.narrow(panned_center, 0.5)  # 50% width
...
buf2 = pycdp.Buffer.from_memoryview(samples2, channels=1, sample_rate=sample_rate)

# Mix two buffers
mixed = pycdp.mix2(buf, buf2, gain_a=0.7, gain_b=0.3)
print(f"Mixed two buffers with gains 0.7 and 0.3")

# Mix multiple buffers
multi_mixed = pycdp.mix([buf, buf2, reversed_buf], gains=[0.5, 0.3, 0.2])
...
print("\n=== File I/O ===")
print("To read a file:  buf = pycdp.read_file('input.wav')")
print("To write a file: pycdp.write_file('output.wav', buf)")
...
outputs = {
    "01_sine_440hz.wav": pycdp.normalize(buf),
    "01_normalized.wav": normalized,
    "01_reversed.wav": pycdp.normalize(reversed_buf),
    "01_faded.wav": pycdp.normalize(buf_copy),
    "01_panned_center.wav": pycdp.normalize(panned_center),
    "01_auto_panned.wav": pycdp.normalize(auto_panned),
    "01_mixed.wav": pycdp.normalize(mixed),
}
