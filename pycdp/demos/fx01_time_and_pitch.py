...
def main():
    ...
    stretched_4x = pycdp.time_stretch(buf, factor=4.0)
    ...
    shifted_up_100 = pycdp.spectral_shift(buf, shift_hz=100.0)
    print(f"Shift +100 Hz (metallic)")
    outputs["fx01_spectral_shift_up.wav"] = shifted_up_100

    shifted_down_50 = pycdp.spectral_shift(buf, shift_hz=-50.0)
    ...
    # Slow and low
    slow_low = pycdp.time_stretch(buf, factor=2.0)
    slow_low = pycdp.pitch_shift(slow_low, semitones=-5.0)
    print(f"Slow + low (2x stretch, -5 semitones)")
    outputs["fx01_slow_and_low.wav"] = slow_low

    # Fast and high
    fast_high = pycdp.time_stretch(buf, factor=0.5)
    fast_high = pycdp.pitch_shift(fast_high, semitones=5.0)
