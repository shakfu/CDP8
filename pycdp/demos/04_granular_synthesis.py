...
# Repitch grains
repitched = pycdp.grain_repitch(buf, pitch_semitones=5.0)
print(f"Repitched grains: +5 semitones")

# Variable pitch with curve
repitched_curve = pycdp.grain_repitch(buf, pitch_curve=[
...
stretched_texture = pycdp.wrappage(buf,
    grain_size=40.0,
    density=2.0,
    velocity=0.5,         # Half speed = 2x stretch
    spread=0.8
...
# Pitched texture
pitched_texture = pycdp.wrappage(buf,
    grain_size=35.0,
    density=2.0,
    velocity=1.0,
...
# Dense cloud texture
dense_cloud = pycdp.wrappage(buf,
...
grain_cloud = pycdp.grain_cloud(buf, gate=0.1, grainsize_ms=30.0)
print(f"Grain cloud: {grain_cloud.frame_count} frames")

# Extend grains into sustained texture
extended = pycdp.grain_extend(buf, grainsize_ms=20.0, trough=0.3)
...
outputs = {
    "04_source.wav": pycdp.normalize(buf),
    "04_brassage_stretch.wav": pycdp.normalize(stretched),
    "04_brassage_pitch.wav": pycdp.normalize(pitched),
    "04_granular_cloud.wav": pycdp.normalize(cloud),
    "04_grain_shuffle.wav": pycdp.normalize(shuffled),
    "04_grain_reverse.wav": pycdp.normalize(reversed_grains),
    "04_grain_repitch.wav": pycdp.normalize(repitched),
    "04_wrappage_basic.wav": pycdp.normalize(texture),
    "04_wrappage_frozen.wav": pycdp.normalize(frozen),
    "04_wrappage_dense.wav": pycdp.normalize(dense_cloud),
    "04_grain_extend.wav": pycdp.normalize(extended),
}
