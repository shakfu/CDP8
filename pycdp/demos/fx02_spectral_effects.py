...
def main():
    ...
    blur_light = pycdp.spectral_blur(buf, blur_time=0.05)
    print(f"Light blur (50ms)")
    outputs["fx02_blur_light.wav"] = blur_light

    blur_heavy = pycdp.spectral_blur(buf, blur_time=0.2)
    print(f"Heavy blur (200ms) - dreamy")
    outputs["fx02_blur_heavy.wav"] = blur_heavy

    blur_extreme = pycdp.spectral_blur(buf, blur_time=0.5)
    ...
    focus_low = pycdp.spectral_focus(buf, center_freq=200.0, bandwidth=100.0, gain_db=6.0)
    print(f"Focus on lows (200 Hz)")
    outputs["fx02_focus_low.wav"] = focus_low

    focus_mid = pycdp.spectral_focus(buf, center_freq=1000.0, bandwidth=300.0, gain_db=6.0)
    print(f"Focus on mids (1000 Hz)")
    outputs["fx02_focus_mid.wav"] = focus_mid

    focus_high = pycdp.spectral_focus(buf, center_freq=4000.0, bandwidth=1000.0, gain_db=6.0)
    ...
    fold_low = pycdp.spectral_fold(buf, fold_freq=1000.0)
    print(f"Fold at 1000 Hz (metallic)")
    outputs["fx02_fold_1k.wav"] = fold_low

    fold_high = pycdp.spectral_fold(buf, fold_freq=3000.0)
    ...
    spec_stretch = pycdp.spectral_stretch(buf, max_stretch=1.5, freq_divide=500.0)
    print(f"Spectral stretch (bell-like)")
    outputs["fx02_spectral_stretch.wav"] = spec_stretch

    spec_stretch_extreme = pycdp.spectral_stretch(buf, max_stretch=3.0, freq_divide=300.0)
    ...
    cleaned = pycdp.spectral_clean(buf, threshold_db=-50.0)
    ...
    # Freeze early part
    freeze_early = pycdp.freeze(buf, start_time=min(0.5, duration * 0.1),
    ...
    # Freeze middle
    mid_time = duration / 2
    freeze_mid = pycdp.freeze(buf, start_time=mid_time, end_time=mid_time + 0.5, duration=8.0)
    print(f"Freeze middle section -> 8s")
    outputs["fx02_freeze_mid.wav"] = freeze_mid

    # Freeze with scatter
    freeze_scatter = pycdp.freeze(buf, start_time=duration * 0.25, end_time=duration * 0.35,
    ...
    dark_ambient = pycdp.time_stretch(buf, factor=3.0)
    dark_ambient = pycdp.spectral_blur(dark_ambient, blur_time=0.15)
    dark_ambient = pycdp.pitch_shift(dark_ambient, semitones=-7.0)
