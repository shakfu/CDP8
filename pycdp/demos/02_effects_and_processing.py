...
# Simple delay
delayed = pycdp.delay(buf, delay_ms=250.0, feedback=0.4, mix=0.5)
...
# Reverb effect
reverbed = pycdp.reverb(buf, decay_time=2.0, damping=0.5, mix=0.4)
...
tremolo = pycdp.tremolo(buf, freq=6.0, depth=0.7)
print(f"Applied tremolo: 6 Hz, 70% depth")

# Chorus
chorus = pycdp.chorus(buf, rate=1.5, depth_ms=20.0, mix=0.5)
print(f"Applied chorus")

# Flanger
flanged = pycdp.flanger(buf, rate=0.5, depth_ms=5.0, feedback=0.5)
print(f"Applied flanger")

# Ring modulation
ring = pycdp.ring_mod(buf, freq=150.0)
...
# Waveset repeat
repeated = pycdp.distort_repeat(buf, multiplier=2)
print(f"Applied waveset repeat: 2x")

# Waveset reverse
wrev = pycdp.distort_reverse(buf, cycle_count=4)
print(f"Applied waveset reverse: 4 cycles")

# Fractal distortion
fractal = pycdp.distort_fractal(buf, scaling=0.5)
print(f"Applied fractal distortion: scaling=0.5")

# Bitcrush
crushed = pycdp.bitcrush(buf, bit_depth=8)
...
dynamic_buf = pycdp.Buffer.from_memoryview(dynamic_samples, channels=1, sample_rate=44100)

# Compression
compressed = pycdp.compressor(dynamic_buf, threshold_db=-12.0, ratio=4.0)
print(f"Applied compression: -12dB threshold, 4:1 ratio")

# Gate
gated = pycdp.gate(dynamic_buf, threshold_db=-20.0)
print(f"Applied gate: -20dB threshold")

# Limiter
limited = pycdp.limiter(dynamic_buf, threshold_db=-3.0)
...
# Low-pass filter
lowpass = pycdp.filter_lowpass(buf, cutoff_freq=1000.0)
print(f"Applied low-pass filter: 1000 Hz cutoff")

# High-pass filter
highpass = pycdp.filter_highpass(buf, cutoff_freq=500.0)
print(f"Applied high-pass filter: 500 Hz cutoff")

# Band-pass filter (low_freq and high_freq)
bandpass = pycdp.filter_bandpass(buf, low_freq=600.0, high_freq=1000.0)
print(f"Applied band-pass filter: 600-1000 Hz")

# Notch filter
notch = pycdp.filter_notch(buf, center_freq=1000.0, width_hz=100.0)
...
outputs = {
    "02_source.wav": pycdp.normalize(buf),
    "02_delay.wav": pycdp.normalize(delayed),
    "02_reverb.wav": pycdp.normalize(reverbed),
    "02_tremolo.wav": pycdp.normalize(tremolo),
    "02_chorus.wav": pycdp.normalize(chorus),
    "02_flanger.wav": pycdp.normalize(flanged),
    "02_ring_mod.wav": pycdp.normalize(ring),
    "02_spin.wav": pycdp.normalize(spun),
    "02_flutter.wav": pycdp.normalize(flutter),
    "02_distort_repeat.wav": pycdp.normalize(repeated),
    "02_bitcrush.wav": pycdp.normalize(crushed),
    "02_compressed.wav": pycdp.normalize(compressed),
    "02_lowpass.wav": pycdp.normalize(lowpass),
    "02_highpass.wav": pycdp.normalize(highpass),
}
