...
def main():
    ...
    brass_stretch = pycdp.brassage(buf, velocity=0.5, density=1.0, grainsize_ms=50.0)
    print(f"Granular 2x stretch")
    outputs["fx03_brassage_stretch.wav"] = brass_stretch

    brass_compress = pycdp.brassage(buf, velocity=2.0, density=1.0, grainsize_ms=30.0)
    ...
    brass_scatter = pycdp.brassage(buf, velocity=0.5, density=1.5, grainsize_ms=40.0, scatter=0.5)
    print(f"Scattered grains (random placement)")
    outputs["fx03_brassage_scatter.wav"] = brass_scatter

    brass_scatter_dense = pycdp.brassage(buf, velocity=0.25, density=3.0, grainsize_ms=60.0, scatter=0.7)
    ...
    brass_pitch_up = pycdp.brassage(buf, velocity=1.0, density=1.0, grainsize_ms=40.0, pitch_shift=7.0)
    print(f"Granular pitch +7 semitones")
    outputs["fx03_brassage_pitch_up.wav"] = brass_pitch_up

    brass_pitch_down = pycdp.brassage(buf, velocity=1.0, density=1.0, grainsize_ms=40.0, pitch_shift=-12.0)
    ...
    wrap_basic = pycdp.wrappage(buf, grain_size=50.0, density=2.0, velocity=1.0, spread=1.0)
    print(f"Basic wrappage (stereo texture)")
    outputs["fx03_wrappage_basic.wav"] = wrap_basic

    wrap_slow = pycdp.wrappage(buf, grain_size=60.0, density=2.0, velocity=0.3, spread=0.8)
    print(f"Slow wrappage (stretched)")
    outputs["fx03_wrappage_slow.wav"] = wrap_slow

    duration = buf.frame_count / buf.sample_rate
    wrap_frozen = pycdp.wrappage(buf, grain_size=80.0, density=4.0, velocity=0.0,
    ...
    wrap_pitched = pycdp.wrappage(buf, grain_size=40.0, density=2.0, velocity=0.5,
                                  pitch=-12.0, spread=0.7)
    print(f"Pitched wrappage (octave down)")
    outputs["fx03_wrappage_pitched.wav"] = wrap_pitched

    wrap_sparse = pycdp.wrappage(buf, grain_size=30.0, density=0.4, velocity=1.0, spread=1.0)
    ...
    cloud = pycdp.grain_cloud(buf, gate=0.1, grainsize_ms=25.0)
    print(f"Grain cloud")
    outputs["fx03_grain_cloud.wav"] = cloud

    extended = pycdp.grain_extend(buf, grainsize_ms=30.0, trough=0.3)
