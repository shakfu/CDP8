...
step1 = pycdp.time_stretch(perc, factor=8.0)
print(f"  1. Time stretch 8x: {step1.frame_count} frames")

# Step 2: Blur to smooth out artifacts
step2 = pycdp.spectral_blur(step1, blur_time=0.1)
print(f"  2. Spectral blur 0.1s")

# Step 3: Add reverb for space
step3 = pycdp.reverb(step2, decay_time=3.0, damping=0.4, mix=0.5)
print(f"  3. Large reverb")

# Step 4: Fade in/out for smooth loop
pycdp.fade_in(step3, duration=0.5, curve="exponential")
pycdp.fade_out(step3, duration=1.0, curve="exponential")
ambient_pad = step3
...
step2 = pycdp.tremolo(step1, freq=4.0, depth=0.8)
print(f"  2. Rhythmic tremolo at 4 Hz")

# Step 3: Stereo spread with flutter
rhythmic = pycdp.flutter(step2, frequency=2.0, depth=0.5)
...
tone = pycdp.Buffer.from_memoryview(tone_samples, channels=1, sample_rate=44100)
print(f"Starting with simple 150 Hz tone")

# Step 1: Add formant-like filtering - use bandpass to create formant bands
step1 = pycdp.filter_bandpass(tone, low_freq=400.0, high_freq=600.0)
step1b = pycdp.filter_bandpass(tone, low_freq=1400.0, high_freq=1600.0)
step1c = pycdp.filter_bandpass(tone, low_freq=2400.0, high_freq=2600.0)
step1 = pycdp.mix([step1, step1b, step1c], gains=[0.5, 0.3, 0.2])
print(f"  1. Formant filtering (500, 1500, 2500 Hz regions)")

# Step 2: Add pitch variation using hover
step2 = pycdp.hover(step1, frequency=5.0, frq_rand=0.3, loc_rand=0.2)
print(f"  2. Hover for pitch variation")

# Step 3: Time stretch for evolution
vocal_texture = pycdp.time_stretch(step2, factor=2.0)
...
step1 = pycdp.grain_reorder(source)  # Random shuffle
print(f"  1. Shuffle grains")

# Step 2: Splinter for stuttering
step2 = pycdp.splinter(step1, start=0.0, duration_ms=50.0, repeats=8, min_shrink=0.3)
print(f"  2. Splinter effect")

# Step 3: Distortion for edge
glitched = pycdp.distort_repeat(step2, multiplier=2)
...
step1 = pycdp.pitch_shift(pad, semitones=12.0)
print(f"  1. Shift up octave")

# Step 2: Wrappage for spatial texture
step2 = pycdp.wrappage(step1,
    grain_size=80.0,
    density=3.0,
    velocity=0.3,
    spread=1.0,
...
# Step 3: Large reverb
step3 = pycdp.reverb(step2, decay_time=4.0, damping=0.2, mix=0.6)
print(f"  3. Large hall reverb")

# Step 4: Spectral blur for softness
ethereal = pycdp.spectral_blur(step3, blur_time=0.08)
...
2. NORMALIZE BEFORE OUTPUT:
   result = pycdp.normalize(result, target=0.9)
   # Or in dB:
   result = pycdp.normalize_db(result, target_db=-1.0)

3. CHECK PEAK LEVELS:
   peak_val, peak_pos = pycdp.peak(buf)
   print(f"Peak level: {peak_val} at sample {peak_pos}")

4. MONO/STEREO CONVERSION:
   # Mono to stereo (centered):
   stereo = pycdp.pan(mono_buf, 0.0)

   # Stereo to mono:
   mono = pycdp.to_mono(stereo_buf)

   # Reduce stereo width:
   narrowed = pycdp.narrow(stereo_buf, 0.5)
...
5. PROCESSING STEREO FILES:
   ...
   - Split, process, recombine:
     left, right = pycdp.split_channels(stereo_buf)
     left_proc = pycdp.some_effect(left)
     right_proc = pycdp.some_effect(right)
     result = pycdp.merge_channels(left_proc, right_proc)

6. AVOID CLIPPING:
   # Always normalize or reduce gain before writing
   peak_val, _ = pycdp.peak(result)
   if peak_val > 1.0:
...
outputs = {
    "06_ambient_pad.wav": pycdp.normalize(ambient_pad),
    "06_rhythmic_texture.wav": pycdp.normalize(rhythmic),
    "06_vocal_texture.wav": pycdp.normalize(vocal_texture),
    "06_glitch.wav": pycdp.normalize(glitched),
    "06_ethereal.wav": pycdp.normalize(ethereal),
}
